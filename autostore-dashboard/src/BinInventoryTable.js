...
export default function BinInventoryTable() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('http://localhost:8000/bins/').then(res => res.json()),
  ...
  const handleRefillSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/products/refill-multiple', {
        method: 'POST',
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  <div className="mb-2 text-gray-700"><b>Status:</b> {bin.status}</div>
                  <div className="mb-2 text-gray-700"><b>Number of Products:</b> {binProducts.reduce((sum, p) => sum + (p.quantity ?? 0), 0)}</div>
                  <div className="mb-2 text-gray-700"><b>Products in Bin:</b></div>
                  {binProducts.length > 0 ? (
                    ...
                    <div className="text-gray-400 text-sm mb-4">No products in this bin.</div>
                  )}
